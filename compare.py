...
def compare(dir1, dir2):
    ...
    d1_len = len([entry for entry in os.listdir(d1) if os.path.isfile(os.path.join(d1, entry))])
    d2_len = len([entry for entry in os.listdir(d2) if os.path.isfile(os.path.join(d2, entry))])
