    ...
    def compare_seeds(self, seed1_path, seed2_path):
        ...
        return int(
            subprocess.check_output(
                diff_str, shell=True, executable="/bin/bash"
            ).split()[0]
        )
    ...
    def compare(self):
        ...
        if self.reader1.index == self.reader2.index:
            ...
            if min_len < 2:
                ...
                check1 = (
                    self.reader1.checkpoints[min_len - 1]
                    == self.reader2.checkpoints[min_len - 1]
                )
                check2 = (
                    self.reader1.checkpoints[min_len - 1]
                    == self.reader2.checkpoints[min_len - 1]
...
def check_scaling_governor():
    sg = subprocess.check_output(
        "cat /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor", shell=True
    ).split()
